...
def run_instance(instance: int, allow_rotation=False, use_chuffed=False, timeout=5, visualize=True):
    ...
    # Set the variables
    problem_instance['W'] = width
    problem_instance['n'] = n
    problem_instance['dimensions'] = dimensions
    ...
    chip_w = [dimensions[i][0] for i in range(n)]
    chip_h = [dimensions[i][1] for i in range(n)]
    if allow_rotation:
        r = result['r']
        print(f'{r=}')
        for i in range(n):
    ...
    print(f'\nSolution: height = {height} (found in {delta_t:.3f} s)')
